    ...
    for attempt in range(3):
        ...
        try:
            ...
            response = client.models.generate_content(
                model="gemini-3.1-flash-lite", # 指定モデルを維持
...
def render_results(all_results_rows):
    ...
    # 第2段：連結販売台数 (追加セクション)
    st.subheader("📈 連結販売台数比較 (H1実績)")
    c3, c4 = st.columns([2, 1])
    
    with c3:
        fig_vol = go.Figure()
        # 他のグラフとトーンを合わせ、販売台数用に青・水色系を採用（淡色：Prior, 濃色：Current）
        fig_vol.add_trace(go.Bar(name='Prior Year', x=df_prior["Company"], y=df_prior["Total Vol"], marker_color='#AED6F1'))
        fig_vol.add_trace(go.Bar(name='Current Year', x=df_current["Company"], y=df_current["Total Vol"], marker_color='#2E86C1'))
        fig_vol.update_layout(title="<b>Global Sales Volume</b> (Units)", barmode='group', template="plotly_white")
        st.plotly_chart(fig_vol, use_container_width=True)

    with c4:
        st.markdown("##### Current Year 販売台数内訳")
        vol_df = df_current[["Company", "Total Vol"]].copy().rename(columns={"Total Vol": "Volume (H1)"})
        st.dataframe(
            vol_df.style.format({"Volume (H1)": "{:,.0f}"}),
